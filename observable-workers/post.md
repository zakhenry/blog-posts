...
published: false
description: "Introduction to `observable-webworker`, a library to help make super clean streaming webworker implementations"
tags: webworker, observable, rxjs, angular, typescript, streaming
series: observable-webworkers
---

## tl;dr
Web Workers are awesome, Angular CLI now supports them natively, the API of Web Workers is not Angular-like, I'm introducing a library [`observable-webworker`](https://www.npmjs.com/package/observable-webworker) which makes the API feel like native Angular, and able to use RxJS streams to make it awesome.

If you already know what webworkers are and use them in Angular, feel free to [skip ahead](#implementing-observablewebworker) to refactoring the initial worker generated by the CLI to a superpowered worker with `observable-webworker`

## Context
In this article series we will explore using [Web Workers](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API/Using_web_workers) to offload heavy computation to another thread, keeping the main thread free for interaction with the user. We don't typically think of threading very often in the frontend world, as non-blocking APIs and the event loop model of Javascript typically allows us to have execution of user interaction events _in-between_ longer running processes such as waiting for an HTTP request to resolve.
...
Angular CLI will now update `tsconfig.app.json` and `angular.json` to enable webworker support, and create us a new `demo.worker.ts`:
...
Before we worry about how to destroy a worker, let's reflect for a bit on the API that we have with workers so far - we have to construct a worker, then declare a property and assign a function to it in order to get the response back from the worker. Additionally within the worker itself, we have to `addEventListener` then call a global `postMessage` function to send back information to the main thread. This API doesn't feel very Angular-like does it? We're used to dealing with clean hook-based APIs and have fallen in love with RxJS for dealing with streams of data and asynchronous responses, why can't we have this for Web Workers too?
...
I should note that this library doesn't actually depend on Angular at all, and will work beautifully with React or any other framework or lack thereof! The only dependency is a `peerDependency` on RxJS.
 ...
 To best introduce the concepts of the library, we will refactor our current webworker to utilise `observable-webworker`.
...
Note we've imported `fromWorker` from `observable-webworker`. The first argument is a worker _factory_ - we need to have the ability to lazily construct a worker on-demand, so we pass a factory so the library can construct it when needed. Also Webpack needs to find the `new Worker('./path/to/file.worker')` in the code in order to be able to bundle it for us.
