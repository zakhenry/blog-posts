...
* the user get's their search results live with every keystroke
...
/**
 * First off, we create a quick enum of the books we will use as a demo. Later we will update the urls to be correct
 */
enum BookChoice {
...
/**
 * This observable will be something like a regular dropdown, emitted whenever the user changes their selection.
 */
const userBookSelection$ = of(BookChoice.ALICE_IN_WONDERLAND);

/**
 * This observable represents the stream of search terms the user will enter into a text box. We `shareReplay(1)`
 */
const userSearchTerm$ = of(`we’re all mad here`).pipe(shareReplay(1));

/**
 * This function will be responsible for fetching the content of the book given the enum. We're cheating a little by
 * making the enum value the url to fetch. For now we will just pretend the url is the content of the book.
 * @todo implement
 */
...
/**
 * This function will be responsible for taking the search term and returning the stream of paragraphs found, as soon as
 * they are found.
 *
 * For the purposes of quick testing, we've hardcoded as if two search results were found
 *
 * We will also likely extend this return type in future to handle highlighting the search phrase, but for now just the
 * paragraph is sufficient for testing
 * @todo implement
 */
...
/**
 * Here we take the user selected book stream and pipe it via switchMap to fetch the content of the book. We use
 * switchMap() because we want to cancel the download of the book if the user switches to a different book to search
 * before the book has finished downloading.
 *
 * Next we again switchMap() the result of the book content to the user search term observable so that if the user has
 * changed books, once it is loaded we will cancel the processing of the current search term.
 *
 * Next we pass that stream of search terms to getSearchResults() which will itself be returning a stream of search
 * results for that search string.
 *
 * Finally, we use a scan() operator to collate the stream of search results into an array so that we can present all
 * results to the user, not just the most recent one
 */
...
/**
 * Last but not least, to check our logic, we subscribe to the observable and bind the console so we see output.
 */
...
```txt
getBookText called (http://some-url-to-alice-in-wonderland-text)
0 : Search Result: [w (this will be the first result)] (+0.231 seconds)
1 : Search Result: [w (this will be the first result), w (this will be the second result)] (+0.001 seconds)
2 : Search Result: [we (this will be the first result)] (+0.173 seconds)
3 : Search Result: [we (this will be the first result), we (this will be the second result)] (+0 seconds)
4 : Search Result: [we’ (this will be the first result)] (+0.177 seconds)
5 : Search Result: [we’ (this will be the first result), we’ (this will be the second result)] (+0 seconds)
6 : Search Result: [we’r (this will be the first result)] (+0.173 seconds)
7 : Search Result: [we’r (this will be the first result), we’r (this will be the second result)] (+0 seconds)
8 : Search Result: [we’re (this will be the first result)] (+0.173 seconds)
9 : Search Result: [we’re (this will be the first result), we’re (this will be the second result)] (+0 seconds)
10 : Search Result: [we’re  (this will be the first result)] (+0.177 seconds)
11 : Search Result: [we’re  (this will be the first result), we’re  (this will be the second result)] (+0 seconds)
12 : Search Result: [we’re a (this will be the first result)] (+0.175 seconds)
13 : Search Result: [we’re a (this will be the first result), we’re a (this will be the second result)] (+0 seconds)
14 : Search Result: [we’re all mad here (this will be the first result)] (+1.896 seconds)
15 : Search Result: [we’re all mad here (this will be the first result), we’re all mad here (this will be the second result)] (+0 seconds)
getBookText called (http://some-url-to-sherlock-holmes-text)
16 : Search Result: [we’re all mad here (this will be the first result)] (+1.398 seconds)
17 : Search Result: [we’re all mad here (this will be the first result), we’re all mad here (this will be the second result)] (+0 seconds)
